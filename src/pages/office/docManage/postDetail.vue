          ...
          <a-tab-pane key="3" tab="签收情况">
            ...
            >
              <span slot="operate" slot-scope="text, record">
                <template v-if="!record.promoter">
                  <a @click="handleRush(record)">催办</a>
                </template>
              </span>
            </a-table>
          </a-tab-pane>
          <a-tab-pane key="4" tab="流程流转">
            <div class="box_art">
              <s-table
                ref="table"
                size="small"
  ...
  methods: {
    ...
    edit() {
      ...
      this.$post("/office/wps/getViewUrlWebPath", params)
        .then((res) => {
          console.log("res-postDetail", res);
    ...
    getData() {
      this.$post(this.url.getPageSet, this.masterQueryParam).then((res) => {
        var list = {
          pageNo: 1,
          pageSize: 10,
          rows: [
        ...
        pager.total = list.totalCount;
        this.dataSource = list.rows;
        this.pagination = pager;
      });
    },
    handleRush(record) {
      this.$message.info("催办了");
    },
  },
};
</script>
