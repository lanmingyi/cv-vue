	...
	<div class="page-common-layout">
		<a-layout>
			<a-layout-sider :width="leftWidth" style="" class="layout-box">
                ...
                ">
								<a-menu-item key="1">
									<a-icon type="plus" /> 新增主任务
								</a-menu-item>
								<a-menu-item key="2">
									<a-icon type="plus" /> 新增子任务
								</a-menu-item>
							</a-menu>
            ...
            icon="edit" @click="handleEdit('all',$refs.masterList.getListData())">编辑</a-button> -->
				...
				<div style="height: calc(100% - 40px - 44px);">
					<tree-grid ref="masterList" :scroll="scrollXY" :columns="columns" rowKey="uuid" :url="url" :parameter="parameter"
						:pagination="pagination" @onSelectChange="onSelectChangeMaster"
						@rowClick="(record, index) => masterRowClick('main', record, index)">
						...
						<span slot="action" slot-scope="{ record }">
							<template>
								<a @click.stop="masterRowClick('second', record)">详情</a>
							</template>
						</span>
					</tree-grid>
				</div>
	...
	export default {
		...
		data() {
			return {
				...
				url: {
					fathersUrl: "/office/oaTaskBase/getMyTaskBaseByLevelId",
					expandUrl: "/office/oaTaskBase/getListByPid",
					masterSaveRoot: "/office/oaTaskBase/save",
					masterSave: "/office/oaTaskBase/saveSon",
		...
		methods: {
			...
			masterRowClick(type, record, index) {
				const that = this;
