		...
		<div class="page-common-content">
			<div class="page-common-box-content">
				<tool-bar @search="$refs.masterList.refresh(true)" @reset="
				    () => {
				      (this.masterQueryParam = {emailSendState:'发送'}), $refs.masterList.refresh(true);
				    }
				  ">
					<template slot="toolBtn" slot-scope="scope">
						<a-button class="cu-btn-primary" icon="plus" @click="handleAdd('master')">写邮件</a-button>
	...
	export default {
		...
		data() {
			return {
				...
				subMdl: null,
				// 查询参数
				masterQueryParam: {emailSendState:'发送'},
				...
				url: {
					...
					masterSaveEmail: "/office/oaEmail/saveEmail",
		...
		methods: {
			...
			handleBrowse(record) {
				...
				if(this.$refs.detailModal.edit) this.$refs.detailModal.edit(record)
				form.resetFields();
			},
			...
			handleJsonOk(e){
				...
				// console.log("form", form); uploadUuidList
				...
				for(var i = 0 ; i < fileList.length ; i++){
					// console.log(fileList[i].response.dfsFileUuid)
					if(i==0) {
				...
				if(e == "save"){
					...
					this.masterUrl = this.url.masterAdd;
				} else {
					fromValue.emailSendState = "发送"
					this.masterUrl = this.url.masterSaveEmail;
				}
