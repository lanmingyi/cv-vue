			...
			<a-col>
				<div v-if="actionButton" class="action-button">
					...
					<template v-if="selectedRowIds.length > 0">
						...
						<a-button type="primary" icon="minus" @click="handleConfirmDelete">删除</a-button>
		...
		<div :id="`${caseId}inputTable`" class="input-table">
			...
			<div class="thead" ref="thead">
				<div class="tr" :style="{ width: this.realTrWidth }">
					...
					<template v-for="col in columns">
						<div v-show="col.type !== formTypes.hidden" class="td" :key="col.key"
			...
			<div class="scroll-view" ref="scrollView" :style="{ 'max-height': maxHeight + 'px' }">
				...
				<div :id="`${caseId}tbody`" class="tbody" :style="tbodyStyle">
					<!-- 扩展高度 -->
					<div class="tr-expand" :style="
              `height:${getExpandHeight}px; z-index:${loading ? '11' : '9'};`
            "></div>
					<!-- 无数据时显示 -->
					...
					<!-- v-model="rows"-->
					<draggable :value="rows" handle=".td-ds-icons" @start="handleDragMoveStart"
                ...
                " :id="`${caseId}tbody-tr-${rowIndex}`" :data-idx="rowIndex" class="tr" :class="
                  selectedRowIds.indexOf(row.id) !== -1 ? 'tr-checked' : ''
                " :style="buildTrStyle(rowIndex)" :key="row.id" @click="handleClickTableRow">
								<!-- 左侧固定td  -->
								...
								<div v-if="dragSort" class="td td-ds" :style="style.tdLeft"
									...
									<a-dropdown :trigger="['click']" :getPopupContainer="getParentContainer">
										...
										<a-menu slot="overlay">
											...
											<a-menu-divider />
											<a-menu-item key="3" @click="_handleRowInsertDown(rowIndex)">插入一行
											</a-menu-item>
										</a-menu>
									</a-dropdown>
								</div>
								...
								<!-- 右侧动态生成td -->
								<div class="td" v-for="col in columns" v-show="col.type !== formTypes.hidden"
									:key="col.key" :style="buildTdStyle(col)">
									<!-- 此 v-for 只是为了拼接 id 字符串 -->
                      ...
                      ">
											<a-tooltip v-bind="buildTooltipProps(row, col, id)">
												<input v-if="isEditRow(row, col)" :id="id" v-bind="buildProps(row, col)"
													:data-input-number="
                            col.type === formTypes.inputNumber
                          " :placeholder="replaceProps(col, col.placeholder)" @blur="
                            (e) => {
                          ...
                          " />
												<span v-else class="j-td-span no-edit"
										...
										<template v-else-if="col.type === formTypes.checkbox">
											<a-checkbox :key="i" :id="id" v-bind="buildProps(row, col)"
												:checked="getCheckValue(checkboxValues[id])" @change="
                          (e) =>
                            handleChangeCheckboxCommon(e, row, col, rowIndex)
                        " />
										</template>
										<!-- select -->
										<template v-else-if="col.type === formTypes.select">
											<a-tooltip v-bind="buildTooltipProps(row, col, id)">
												<a-select v-if="isEditRow(row, col)" :id="id" :key="i"
													...
													:value="selectValues[id]" :getPopupContainer="getParentContainer"
													:placeholder="replaceProps(col, col.placeholder)" :filterOption="
                            (i, o) => handleSelectFilterOption(i, o, col)
                          " @change="
                            (v) =>
                              handleChangeSelectCommon(
                                v,
                                id,
                                row,
                                col,
                          ...
                          " @search="(v) => handleSearchSelect(v, id, row, col)"
													...
													<a-select-option v-for="item in col.options" :key="item.value"
														:value="
                              col.valueFeild ? item[col.valueFeild] : item.value
                            ">
														{{
                              col.textFeild ? item[col.textFeild] : item.text
                            }}
													</a-select-option>
												</a-select>
												<span v-else class="j-td-span no-edit"
                      ...
                      ">
											<a-tooltip v-bind="buildTooltipProps(row, col, id)">
												<t-date v-if="isEditRow(row, col)" :id="id" :key="i"
													v-bind="buildProps(row, col)" style="width: 100%;"
													:value="jdateValues[id]" :getCalendarContainer="getParentContainer"
													:placeholder="replaceProps(col, col.placeholder)"
													:trigger-change="true" :showTime="col.type === formTypes.datetime"
													:dateFormat="
                            col.type === formTypes.date
                              ? 'YYYY-MM-DD'
                              : 'YYYY-MM-DD HH:mm:ss'
                          ...
                          " />
										...
										<template v-else-if="col.type === formTypes.radio">
											<a-tooltip v-bind="buildTooltipProps(row, col, id)">
												<a-radio-group :id="id" :key="i" v-bind="buildProps(row, col)"
													:value="radioValues[id]" @change="
                            (e) =>
                              handleRadioChange(
                                e.target.value,
                                id,
                          ...
                          ">
										...
										<template v-else-if="col.type === formTypes.list_multi">
											<a-tooltip v-bind="buildTooltipProps(row, col, id)">
												<a-select v-if="isEditRow(row, col)" :id="id" :key="i" mode="multiple"
													:maxTagCount="1" v-bind="buildProps(row, col)" style="width: 100%;"
													:value="multiSelectValues[id]"
													:getPopupContainer="getParentContainer"
													:placeholder="replaceProps(col, col.placeholder)" @change="
                            (v) =>
                              handleMultiSelectChange(v, id, row, col, rowIndex)
                          " allowClear>
													<a-select-option v-for="item in col.options" :key="item.value"
														:value="
                              col.valueFeild ? item[col.valueFeild] : item.value
                            ...
                            getSelectTranslateText(
                              ...
                              col
										...
										<template v-else-if="col.type === formTypes.sel_search">
											<a-tooltip v-bind="buildTooltipProps(row, col, id)">
												<a-select v-if="isEditRow(row, col)" :id="id" :key="i" showSearch
                          ...
                          " allowClear>
                            ...
                            getSelectTranslateText(
                              ...
                              col
										...
										<!-- 数据字典下拉 -begin -->
										<template v-else-if="col.type === formTypes.combbox">
											<a-tooltip v-bind="buildTooltipProps(row, col, id)">
												<TDictSelectTag v-if="isEditRow(row, col)" :id="id" :key="i"
													optionFilterProp="children" :filterOption="filterOption"
													v-bind="buildProps(row, col)" style="width: 100%;"
													:value="searchSelectValues[id]"
													:getPopupContainer="getParentContainer"
                          ...
                          " :textfield="col.textfield" :valueFeild="col.valueFeild" allowClear />
												<span v-else class="j-td-span no-edit"
													:class="{ disabled: buildProps(row, col).disabled }"
													@click="handleEditRow(row, col)">{{
                            getSelectTranslateText(
                              ...
                              col
										...
										<div v-else-if="col.type === formTypes.slot" :key="i">
											<a-tooltip v-bind="buildTooltipProps(row, col, id)">
												<slot :name="col.slot || col.slotName || col.key" :index="rowIndex"
													:text="slotValues[id]" :value="slotValues[id]" :column="col"
													:rowId="getCleanId(row.id)"
													:getValue="() => _getValueForSlot(row.id)" :caseId="caseId"
													:allValues="_getAllValuesForSlot()" :target="getVM()" :handleChange="
                            (v) => handleChangeSlotCommon(v, id, row, col)
              ...
              height: '32px',
						...
						<!-- 右侧动态生成td -->
						<template v-for="col in columns">
							<div :key="col.key" class="td" v-show="col.type !== formTypes.hidden"
	...
	export default {
		...
		methods: {
			...
			generateId(rows) {
				...
				return `${this.caseId}${timestamp}${rows.length}${randomNumber(6)}${
        this.tempId
      }`;
			},
			/** push 一条数据 */
				...
				dataSource.forEach((data, newValueIndex) => {
					...
					this.columns.forEach((column) => {
						...
						if (setDefaultValue) {
							...
							sourceValue =
								typeof sourceValue === "number" || sourceValue ?
								sourceValue :
								defaultValue;
						}
						...
						if (column.type === FormTypes.inputNumber) {
							...
							if (sourceValue) {
								...
								if (sourceValue.indexOf(",") > 0) {
									...
									fileName = sourceValue2.substring(
										sourceValue2.lastIndexOf("/") + 1
									);
								} else {
									fileName = sourceValue.substring(
					...
					if (-1 !== rows.findIndex((item) => item.id === row.id)) {
						added = true;
						this.inputValues = this.inputValues.map((item) =>
							item.id === row.id ? value : item
						);
					}
					//update-begin-author:lvdandan date:20201105 for:LOWCOD-987 【online】js增强的问题--数据对象带有id，且和现有数据一致时，替换患有数据
					if (!added) {
			...
			add(num = 1, forceScrollToBottom = false) {
				...
				console.log("!(tbody.scrollHeight - realScrollTop <= 10)",tbody.scrollTop,tbody.scrollHeight)
			...
			removeRows(id) {
				...
				ids.forEach((removeId) => {
					...
					const findAndDelete = (arr) => {
						for (let i = 0; i < arr.length; i++) {
							...
							if (currentId === removeId) {
								console.log(currentId, removeId, i);
								arr.splice(i, 1);
								index = i;
								return true;
							}
					...
					// this.$emit("deleted", this.getDeleteIds(), this, index);
					// 找到rows对应的index，并删除
				...
				values.forEach((item) => {
					...
					for (let newValueKey in newValues) {
						if (newValues.hasOwnProperty(newValueKey)) {
								...
								if (column.key === newValueKey) {
									...
									if (!edited) {
										...
										if (column.type === FormTypes.select) {
											...
											let key = this.valuesHasOwnProperty(
												this.checkboxValues,
												modelKey
											);
											// 找到对应的column
							...
							if (!edited) {
								console.warn(
									`JEditableTable.setValues：没有找到"${newValueKey}"列`
								);
							}
						}
					}
			...
			recalcAllStatisticsColumns() {
				if (this.hasStatisticsColumn) {
					Object.keys(this.statisticsColumns).forEach((key) =>
						this.recalcOneStatisticsColumn(key)
					);
				}
			},
			// 重新计算单个统计列
			...
			handleInputCommono(target, index, row, column) {
				...
				// 触发valueChange 事件
				if (change) {
					this.elemValueChange(type, row, column, value, index);
				}
			},
			/** slot Change */
			...
			handleChangeCheckboxCommon(event, row, column, index) {
				let {
					id,
					checked
				} = event.target;
				...
				// 触发valueChange 事件
				this.elemValueChange(FormTypes.checkbox, row, column, checked, index);
			},
			handleChangeSelectCommon(value, id, row, column, index) {
				...
				// 触发valueChange 事件
				this.elemValueChange(FormTypes.select, row, column, value, index);
			},
			...
			handleChangeJInputPopCommon(value, id, row, column) {
				this.jInputPopValues = this.bindValuesChange(
					value,
					id,
					"jInputPopValues"
				);
			...
			elemValueChange(type, rowSource, columnSource, value, index) {
          ...
          this.columns.filter((column) => !isEmptyWidth(column)).length}%`;
			...
			handleSearchSelectChange(value, id, row, column, index) {
				...
				this.elemValueChange(FormTypes.sel_search, row, column, value, index);
			},
			...
			handleMouseup(event) {
				...
				for (let p of path) {
					...
					pClassName =
						typeof pClassName === "string" ? pClassName : pClassName.toString();
					...
					// 点击的标签是日期选择器的弹出层
					if (
						pClassName.includes("j-date-picker") ||
						pClassName.includes("ant-calendar-picker-container")
					) {
						return;
					}
				}
