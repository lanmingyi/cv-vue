  ...
  methods: {
    ...
    handleOkFileCatalog() {
      ...
      let destinationKeyList = [];
			...
			console.log(this.$refs.subList.selectedRows)
      this.$refs.subList.selectedRows.forEach((e) => {
        if (e.objectName.indexOf("/") > 0) {
          let arr = e.objectName.split("/");
          destinationKeyList.push(this.queryParamDicItem.keyPrefix + arr[arr.length - 1]);
        } else {
          destinationKeyList.push(this.queryParamDicItem.keyPrefix + e.objectName);
        }
      });
      ...
      this.$post("/document/oss/ossFileMoveFolder", this.fileCatalog)
		...
		// 获取列表
		fetch(params) {
		  this.$post(this.url.getPageSet, this.queryParamDicItem).then(
		    (res) => {
		      const pager = {
		        ...this.pagination,
		      };
