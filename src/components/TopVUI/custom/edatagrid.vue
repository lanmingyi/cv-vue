          ...
          > -->
            ...
            <template v-for="(row,rowIndex) in rows">
              ...
              <div
                ...
                :data-idx="rowIndex"
                class="tr"
                :class="selectedRowIds.indexOf(row.uuid) !== -1 ? 'tr-checked' : ''"
              ...
              >
                ...
                <div v-if="rowSelection" class="td td-cb" :style="style.tdLeft">
                  ...
                  <template v-for="(id,i) in [`${row.uuid}`]">
                    <a-checkbox
                      :id="id"
                      :key="i"
                  ...
                  <template v-for="(id,i) in [`${col.dataIndex}${row.uuid}`]">
                    ...
                    <label :key="i" v-if="col.type === formTypes.input || col.type === formTypes.inputNumber">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        ...
                        >{{ inputValues[rowIndex][col.dataIndex] }}</span>
                      </a-tooltip>
                    </label>
                    <!-- checkbox -->
                    ...
                    <template v-else-if="col.type === formTypes.select">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <a-select
                          ...
                          @change="(v)=>handleChangeSelectCommon(v,id,row,col,index)"
                    ...
                    <template v-else-if="col.type === formTypes.date || col.type === formTypes.datetime">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <t-date
                          ...
                          allowClear
                          @change="(v)=>handleChangeJDateCommon(v,id,row,col,col.type === formTypes.datetime,index)"
                        />
                        <span
                          v-else
                          class="j-td-span no-edit"
                    ...
                    <template v-else-if="col.type === formTypes.popup">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <t-popup
                          ...
                          :field="col.field || col.dataIndex"
                          :org-fields="col.orgFields"
                          :dest-fields="col.destFields"
                          :code="col.popupCode"
                          :groupId="caseId"
                          @input="(value,others)=>popupCallback(value,others,id,row,col,rowIndex,index)"
                        />
                        <span
                          v-else
                          class="j-td-span no-edit"
                      ...
                      <div :hidden="uploadValues[id] != null">
                        <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                          <a-upload
                            ...
                            v-bind="buildProps(row,col)"
                            @change="(v)=>handleChangeUpload(v,id,row,col,index)"
                          >
                    ...
                    <template v-else-if="col.type === formTypes.radio">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <a-radio-group
                          ...
                          @change="(e)=>handleRadioChange(e.target.value,id,row,col,index)">
                    ...
                    <template v-else-if="col.type === formTypes.list_multi">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <a-select
                          ...
                          :getPopupContainer="getParentContainer"
                          :placeholder="replaceProps(col, col.placeholder)"
                          @change="(v)=>handleMultiSelectChange(v,id,row,col,index)"
                          allowClear
                        />
                        <span
                          v-else
                          ...
                          :class="{disabled: buildProps(row,col).disabled}"
                          @click="handleEditRow(row, col)"
                        >{{ getSelectTranslateText(multiSelectValues[id], row, col,index) }} </span>
                      </a-tooltip>
                    </template>
                    <!-- select多选 -end -->
                    ...
                    <template v-else-if="col.type === formTypes.sel_search">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <a-select
                          ...
                          :getPopupContainer="getParentContainer"
                          :placeholder="replaceProps(col, col.placeholder)"
                          @change="(v)=>handleSearchSelectChange(v,id,row,col,index)"
                          allowClear
                        />
                        <span
                          v-else
                          ...
                          :class="{disabled: buildProps(row,col).disabled}"
                          @click="handleEditRow(row, col)"
                        >{{ getSelectTranslateText(searchSelectValues[id], row, col,index) }}</span>
                      </a-tooltip>
                    </template>
                    <!-- select搜索 -end -->
                    ...
                    <div v-else-if="col.type === formTypes.slot" :key="i">
                      <a-tooltip v-bind="buildTooltipProps(row, col, id)">
                        <slot
                          :name="(col.slot || col.slotName) || col.dataIndex"
                          :index="rowIndex"
                          :text="slotValues[id]"
                          :value="slotValues[id]"
                          :column="col"
                          :rowId="getCleanId(row.uuid)"
                          :getValue="()=>_getValueForSlot(row.uuid)"
                          :caseId="caseId"
                          :allValues="_getAllValuesForSlot()"
                          :target="getVM()"
                          :handleChange="(v)=>handleChangeSlotCommon(v,id,row,col,index)"
                          :isNotPass="notPassedIds.includes(col.dataIndex+row.uuid)"
                        />
                      </a-tooltip>
                    </div>

                    <!-- else (normal) -->
                    <span v-else :key="i" v-bind="buildProps(row,col)">{{ inputValues[rowIndex][col.dataIndex] }}</span>
                  </template>
                </div>
              </div>
              <!-- tr end   -->
            ...
            <template v-for="col in columns">
              ...
              >
                <span
                  ...
                  style="padding: 0 5px;"
                >{{statisticsColumns[col.dataIndex]}}</span>
              </div>
            </template>
          </div>
        </div>
...
<script>
  ...
  import { FormTypes, VALIDATE_NO_PASSED } from '@/utils/TEditableTableUtil'
  ...
  import { filterDictText, initDictOptions } from '../dict/JDictSelectUtil'
  import { dataListMixin } from '@/mixins/dataListMixin'
  import { baseUrl } from '@/services/baseUrl.js'
  // import { getFileAccessHttpUrl } from '@/services/manage'
  ...
  export default {
    name: 'JEditableTable',
    components: { Draggable, TDate, TPopup },
    provide() {
      return {
    ...
    data() {
      return {
        ...
        style: {
          ...
          tbody: { left: '0px' },
          // 左侧固定td的style
          tdLeft: { 'min-width': '3%', 'max-width': '50px' },
    ...
    methods: {
      ...
      _pushByDataSource(dataSource, insertIndexes = null, update = true, rows = null, setDefaultValue = false) {
        ...
        dataSource.forEach((data, newValueIndex) => {
          ...
          // }
          let row = { uuid: dataId }
          let value = { uuid: dataId }
          let disabled = false
          this.columns.forEach(column => {
            let inputId = column.dataIndex + value.uuid
            let sourceValue = (data[column.dataIndex] == null ? '' : data[column.dataIndex]).toString()
            ...
            if (column.type === FormTypes.inputNumber) {
              ...
              if (column.isOrder === true) {
                value[column.dataIndex] = this.getInputNumberMaxValue(column) + 1
              } else {
                value[column.dataIndex] = sourceValue
              }
              // 判断是否是统计列
              if (column.statistics) {
                this.hasStatisticsColumn = true
                if (!this.statisticsColumns[column.dataIndex]) {
                  this.$set(this.statisticsColumns, column.dataIndex, 0)
                }
              }
      ...
      getInputNumberMaxValue(column) {
        let maxNum = 0
        this.inputValues.forEach((item, index) => {
          let val = item[column.dataIndex], num
          try {
            num = parseInt(val)
          } catch {
            num = 0
      ...
      insert(insertIndex, num = 1) {
        ...
        let newRows = []
        for (let i = 0; i < num; i++) {
          let row = { uuid: this.generateId(rows) }
        ...
        ids.forEach(removeId => {
          ...
          const findAndDelete = (arr) => {
            for (let i = 0; i < arr.length; i++) {
              let currentId = this.getCleanId(arr[i].uuid)
        ...
        for (let value of inputValues) {
          ...
          if (rowIds == null) {
            ...
            for (let rowId of rowIds) {
              if (this.getCleanId(rowId) === this.getCleanId(value.uuid)) {
                rowIdsFlag = true
                break
              }
            }
          ...
          this.columns.forEach(column => {
            ...
            if (column.type === FormTypes.checkbox) {
              ...
              if (column.customValue instanceof Array) {
                value[column.dataIndex] = checked ? column.customValue[0] : column.customValue[1]
              } else {
                value[column.dataIndex] = checked
              }

            } else if (column.type === FormTypes.select) {
              let selected = this.selectValues[inputId]
              if (selected instanceof Array) {
                value[column.dataIndex] = cloneObject(selected)
              } else {
                value[column.dataIndex] = selected
              }

            } else if (column.type === FormTypes.date || column.type === FormTypes.datetime) {
              value[column.dataIndex] = this.jdateValues[inputId]

            } else if (column.type === FormTypes.input_pop) {
              value[column.dataIndex] = this.jInputPopValues[inputId]

            } else if (column.type === FormTypes.upload) {
              value[column.dataIndex] = cloneObject(this.uploadValues[inputId] || null)
              ...
              if (currUploadObj) {
                value[column.dataIndex] = currUploadObj['path'] || null
              }

            } else if (column.type === FormTypes.popup) {
              if (!value[column.dataIndex]) {
                value[column.dataIndex] = this.popupValues[inputId] || null
              }
            } else if (column.type === FormTypes.radio) {
              value[column.dataIndex] = this.radioValues[inputId]
            } else if (column.type === FormTypes.sel_search) {
              value[column.dataIndex] = this.searchSelectValues[inputId]
            } else if (column.type === FormTypes.list_multi) {
              if (!this.multiSelectValues[inputId] || this.multiSelectValues[inputId].length === 0) {
                value[column.dataIndex] = ''
              } else {
                value[column.dataIndex] = this.multiSelectValues[inputId].join(',')
              }
            } else if (column.type === FormTypes.slot) {
              value[column.dataIndex] = this.slotValues[inputId]
            }
            ...
            //   }
            //   asyncCount++
            //   let results = this.validateOneInputAsync(value[column.dataIndex], value, column, notPassedIds, false, 'getValues', (results) => {
          ...
          // 删除 tempId
          if (deleteTempId && this.isTempId(value.uuid)) {
            delete value.uuid
          } else {
            value.uuid = this.getCleanId(value.uuid)
          }
          ...
          for (let newValueKey in newValues) {
            if (newValues.hasOwnProperty(newValueKey)) {
              ...
              this.inputValues.forEach(value => {
                // 在inputValues中找到了该字段
                if (rowKey === this.getCleanId(value.uuid)) {
              ...
              if (!edited) {
                ...
                for (let column of this.columns) {
                  if (column.dataIndex === newValueKey) {
                    edited = true
                    // 判断是否设定了customValue（自定义值）
                    if (column.customValue instanceof Array) {
                      let customValue = (column.customValue[0] || '').toString()
      ...
      validateOneInputAsync(value, row, column, notPassedIds, update = false, validType = 'input', callback) {
        let tooltips = Object.assign({}, this.tooltips)
        // let notPassedIds = cloneObject(this.notPassedIds)
        let inputId = column.dataIndex + row.uuid
        ...
        if (typeof passed === 'function') {
          let executed = false
          passed(validType, value, { id: this.getCleanId(row.uuid) }, { ...column }, (flag, msg) => {
            if (executed) return
            executed = true
      ...
      handleChangeCheckedAll() {
        ...
        if (!this.getSelectAll) {
          this.rows.forEach(row => {
            if ((this.disabledRowIds || []).indexOf(row.uuid) === -1) {
              selectedRowIds.push(row.uuid)
            }
          })
        }
      ...
      handleInputCommono(target, index, row, column) {
        let oldValue = this.inputValues[index][column.dataIndex] || ''
      ...
      /** slot Change */
      handleChangeSlotCommon(value, id, row, column,index) {
        this.slotValues = this.bindValuesChange(value, id, 'slotValues')
        this.dataSource[index][column.dataIndex] = value
        // 做单个表单验证
      ...
      handleBlurCommono(target, index, row, column) {
        ...
        this.dataSource[index][column.dataIndex] = target.value
        // console.log(this.dataSource)
        // console.log(row, column)
        // this.masterRows['uuid'] = row.uuid
        // this.masterRows[column.dataIndex] = target.value
        // this.masterRows={
        //   uuid: row.uuid,
        //   column.dataIndex: target.value
        // }
        // this.handleEdiTableEdit(this.masterRows)
        // 做单个表单验证
      ...
      handleChangeSelectCommon(value, id, row, column,index) {
        this.dataSource[index][column.dataIndex] = value
      ...
      handleChangeJDateCommon(value, id, row, column, showTime,index) {
        this.dataSource[index][column.dataIndex] = value
      ...
      handleChangeUpload(info, id, row, column,index) {
        console.log(info, id, row, column,index)
        let { file } = info
        let value = {
          name: file.name,
          type: file.type,
      ...
      handleFileSuccess(obj){
        if(obj.uuid){
          this.uploadValues = this.bindValuesChange(obj, obj.uuid, 'uploadValues')
        }
      },
      // 判定当前行是否是正在编辑的
      isEditRow(row, col, index) {
        ...
        if(cu === true){
          ...
          this.index = index
          this.editIndex = col.dataIndex
          return true
          // this.dataSource[index][col.dataIndex] = target.value
          // console.log(this.dataSource)
          // console.log(row, column)
          // this.masterRows['uuid'] = row.uuid
          // this.masterRows[column.dataIndex] = target.value
          ...
          //   column.dataIndex: target.value
          // }
          // this.handleEdiTableEdit(this.dataSource)
        } else {
          // this.flag = !this.flag
          // console.log(this.index, index)
          if(this.index === index && this.editIndex === col.dataIndex) {
            console.log(this.index, index)
            this.index = -1
            this.handleEdiTableEdit(this.dataSource[index])
          }
        }
        // const editDataList = []
        // editDataList.push(cu)
        // if(this.flag){
        //   this.editDataList = editDataList
        //   this.flag = false
        // }
        // const diff = editDataList.filter(item => !new Set(this.editDataList).has(item))
        // console.log(diff)
        // console.log(!!(current && current[col.dataIndex] === true))
      ...
      handleEditRow(row, col) {
        ...
        if (col.type === FormTypes.input || col.type === FormTypes.inputNumber) {
          this.$nextTick(() => {
            // this.forceUpdateFormValues()
            // 自动获取焦点
            let el = document.getElementById(`${col.dataIndex}${row.uuid}`)
            if (el) {
              el.focus()
            }
          })
      ...
      elemValueChange(type, rowSource, columnSource, value) {
        ...
        row.uuid = this.getCleanId(row.uuid)
        // 获取整行的数据
        let { values } = this.getValuesSync({ validate: false, rowIds: [row.uuid] })
        if (values.length > 0) {
      ...
      replaceProps(col, value) {
        if (value && typeof value === 'string') {
          ...
          value = value.replace(/\${key}/g, col.dataIndex)
      ...
      buildProps(row, col) {
        ...
        if (props['disabled'] !== true) {
          props['disabled'] = ((this.disabledRowIds || []).indexOf(row.uuid) !== -1)
        }
      ...
      popupCallback(value, others, id, row, column, index) {
        ...
        if (others) {
          let rowKey = this.getCleanId(row.uuid)
      ...
      /** popup输入框回显 */
      getPopupValue(id) {
        // console.log(this.popupValues[id])
        return this.popupValues[id]
      },
      handleRadioChange(value, id, row, column,index) {
      ...
      handleMultiSelectChange(value, id, row, column,index) {
      ...
      handleSearchSelectChange(value, id, row, column,index) {
      ...
      /** 获取Select等组件翻译后的文本 */
      getSelectTranslateText(value, row, col,index) {
        // 翻译支持单选和多选（数组、逗号分割）
        
        // this.dataSource[index][col.dataIndex] = value
      ...
      handleMouseup(event) {
        ...
        for (let p of path) {
          ...
          if ((p.uuid || '').startsWith(`${this.caseId}tbody-tr`)) {
            return
          }
          ...
          if (pClassName.includes('ant-input')) {
            return
          }
          // 点击的标签是JPopup的弹出层
          if (pClassName.includes('t-modal')) {
            return
          }
          if (pClassName.includes('')) {
            return
          }
          // 点击的标签是日期选择器的弹出层
...
<style lang="less" scoped>
  ...
  .input-table .thead .td.td-cb,
  .input-table .tbody .td.td-cb,
  .input-table .thead .td,
  .input-table .thead .td.td-num,
