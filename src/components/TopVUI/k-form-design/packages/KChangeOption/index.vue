<!--
 * @Description: 表单组件数据配置动态添加
 * @Author: 黄婷
 * @Date: 2021-06-02 13:38:08
 * @LastEditTime: 2021-09-16 10:47:10
 * @LastEditors: 黄婷
 * @FilePath: \vite-demoe:\wwwroot\Vue\MisBoot\misboot-cloud-vue\src\components\TopVUI\k-form-design\packages\KChangeOption\index.vue
-->
<template>
	<div class="option-change-container">
		<!-- <a-row v-if="type === 'option'" :gutter="8">
			<div class="option-change-box" v-for="(val, index) in value" :key="index">
				<a-col :span="9">
					<a-input v-model="val.text" placeholder="名称" />
				</a-col>
				<a-col :span="9">
		...
		<a-row v-if="type === 'option'" :gutter="6">
			<div class="option-change-box" v-for="(val, index) in value" :key="index">
				<a-col :span="18">
					<a-input v-model="val.text" placeholder="名称" />
				</a-col>
