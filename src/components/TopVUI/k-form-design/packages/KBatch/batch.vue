<!--
 * @Description: 动态表格
 * @Author: 黄婷
 * @Date: 2021-06-02 13:38:08
 * @LastEditTime: 2021-09-16 10:45:15
 * @LastEditors: 黄婷
 * @FilePath: \vite-demoe:\wwwroot\Vue\MisBoot\misboot-cloud-vue\src\components\TopVUI\k-form-design\packages\KBatch\batch.vue
-->
<template>
  <a-form-model
    ref="dynamicValidateForm"
    layout="inline"
    :model="dynamicValidateForm"
  >
    <a-table
      class="batch-table"
      :pagination="false"
      :rowKey="(record) => record.key"
      :columns="columns"
      :dataSource="dynamicValidateForm.domains"
      bordered
    >
		<!-- :scroll="{
		  x: listLength * 190 + 80 + (!record.options.hideSequence ? 60 : 0),
		  y: record.options.scrollY,
		}" -->
      <template
        v-for="item in record.list"
        :slot="item.key"
        slot-scope="text, record, index"
      >
        <KFormModelItem
          :key="item.key + '1'"
          :record="item"
          :config="config"
          :parentDisabled="disabled"
          :index="index"
          :domains="dynamicValidateForm.domains"
          :dynamicData="dynamicData"
          v-model="record[item.model]"
          @input="handleInput"
          :isView="isView"
        />
      </template>
      <template slot="dynamic-delete-button" slot-scope="text, record">
        <a-icon
          v-if="!disabled"
          class="dynamic-delete-button"
          type="minus-circle-o"
          @click="removeDomain(record)"
        />
      </template>
    </a-table>
    <a-button
      type="dashed"
      :disabled="disabled"
      @click="addDomain"
      v-if="!isView"
    >
      <a-icon type="plus" />增加
    </a-button>
  </a-form-model>
</template>

<script>
import KFormModelItem from "./module/KFormModelItem";
import KFormModelItemView from "./module/KFormModelItemView";
export default {
  name: "KBatch",
  props: ["record", "value", "dynamicData", "config", "parentDisabled"],
  components: {
    KFormModelItem,
    KFormModelItemView,
  },
  watch: {
    value: {
      // value 需要深度监听及默认先执行handler函数
      handler(val) {
				console.log("val",val,this.record)
				this.dynamicValidateForm.domains =
				  typeof val === "string" || !val ? (val ? JSON.parse(val) : []) : val;
					// this.dynamicValidateForm.domains.forEach(item => {
					// 	item[Object.keys(item)[0]] = undefined
					// })
      },
      immediate: true,
      deep: true,
    },
  },
  data() {
    return {
      dynamicValidateForm: {
        domains: [],
      },
      isView: false,
    };
  },
  mounted() {
    console.log(this.record.list.find((res) => res.options.disabled === false));
  },
  computed: {
    listLength() {
      return this.record.list.filter((item) => !item.options.hidden).length;
    },
    columns() {
      let columns = [];
      if (!this.record.options.hideSequence) {
        columns.push({
          title: "序号",
          dataIndex: "sequence_index_number",
          width: "50px",
          align: "center",
          customRender: (text, record, index) => {
            return index + 1;
          },
        });
      }

      columns.push(
        ...this.record.list
          .filter((item) => !item.options.hidden)
          .map((item, index) => {
            return {
              title: item.label,
              dataIndex: item.key,
              width: index === this.record.list.length - 1 ? "" : "190px",
              scopedSlots: {
                customRender: item.key,
              },
            };
          })
      );
      const lit = this.record.list.filter(
        (res) => res.options.disabled === false
      );
      console.log(lit, "lit====");
      if (lit.length === 0) {
        this.isView = true;
      } else {
        this.isView = false;
        columns.push({
          title: "操作",
          dataIndex: "dynamic-delete-button",
          fixed: "right",
          width: "80px",
          align: "center",
          scopedSlots: {
            customRender: "dynamic-delete-button",
          },
        });
      }
      return columns;
    },
    disabled() {
      return this.record.options.disabled || this.parentDisabled;
    },
  },
  methods: {
    validationSubform() {
      let verification;
      this.$refs.dynamicValidateForm.validate((valid) => {
        verification = valid;
      });
      return verification;
    },
    resetForm() {
      this.$refs.dynamicValidateForm.resetFields();
    },
    removeDomain(item) {
      const index = this.dynamicValidateForm.domains.indexOf(item);
      if (index !== -1) {
        this.dynamicValidateForm.domains.splice(index, 1);
				this.handleInput();
      }
    },
    addDomain() {
      let data = {};
      this.record.list.forEach((item) => {
				data[item.model] = [];
      });
      this.dynamicValidateForm.domains.push({
        ...data,
        key: Date.now(),
      });
      this.dynamicValidateForm.domains = JSON.stringify(
        this.dynamicValidateForm.domains
      );
      this.handleInput();
    },
    handleInput(e) {
      console.log(this.dynamicValidateForm.domains);
      this.$emit("change", this.dynamicValidateForm.domains);
    },
  },
};
</script>
<style scoped="scoped" lang="less">
.dynamic-delete-button {
  cursor: pointer;
  position: relative;
  top: 4px;
  font-size: 24px;
  color: #999;
  transition: all 0.3s;
}

.dynamic-delete-button:hover {
  color: #e89;
}

.dynamic-delete-button[disabled] {
  cursor: not-allowed;
  opacity: 0.5;
}

.batch-table .ant-form-item-control,
.batch-table .ant-form-item {
  line-height: 22px !important;
  margin-bottom: 0 !important;
}

/deep/ .ant-table-scroll,/deep/ .ant-table-content .ant-table-body{
	overflow: visible !important;
}

/deep/ .ant-table-content{
	overflow: auto !important;
}
</style>
