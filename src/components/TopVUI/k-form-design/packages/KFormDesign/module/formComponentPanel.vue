...
<template>
  <div class="form-panel">
    ...
    <a-form
      ...
      :labelAlign="data.config.labelAlign"
    ...
    >
      ...
      >
        <transition-group
          ...
          name="list"
          class="list-main"
          :class="[data.config.columns === 'two' ? 'flexColumns' : '']"
        >
          <layoutItem
            class="drag-move"
    ...
    handleCopy(isCopy = true, data) {
      const traverse = (array) => {
        array.forEach((element, index) => {
          console.log(element.key, this.selectItem.key);
          if (element.key === this.selectItem.key) {
            ...
            //   };
            //   array.splice(array.length + 1, 0, data);
