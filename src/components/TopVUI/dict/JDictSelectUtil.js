...
import { getDictItemsFromCache, postAxiosRequest } from '@/services/common'
...
export async function initDictOptions(dictCode) {
    ...
    //获取字典数组
    let res = await postAxiosRequest('/system/dicSet/getDicItemByCode', { code: dictCode });
    return res;
}
...
export function filterDictTextByCache(dictCode, key) {
    ...
    //优先从缓存中读取字典配置
    postAxiosRequest('/system/dicSet/getDicItemByCode', { code: dictCode }).then((res) => {
 ...
 * @returns string
 */
export function filterMultiTextByCache(dictCode, key) {
    if (key == null || key.length == 0) {
        return;
    }
    if (!dictCode) {
...
export async function getDictItems(dictCode, params) {
    ...
    //缓存中没有，就请求后台
    return await postAxiosRequest('/system/dicSet/getDicItemByCode', { code: dictCode }, params).then(({ success, result }) => {
        if (success) {
