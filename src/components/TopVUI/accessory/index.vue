 ...
 * @LastEditTime: 2021-09-16 09:09:45
 * @LastEditors: 黄婷
 * @FilePath: \vite-demoe:\wwwroot\Vue\MisBoot\misboot-cloud-vue\src\components\TopVUI\accessory\index.vue
-->
<template>
	<a-card :bordered="false" :bodyStyle="{ padding: 0 }">
		<!-- 操作按钮 -->
		<tool-bar v-if="btnType" @search="$refs.table.refresh(true)" @reset="
        () => {
          (this.queryParam = {}), $refs.table.refresh(true);
        }
      ">
			<template slot="toolBtn" slot-scope="scope">
				<a-button class="cu-btn-primary" v-if="!type || type != 1" @click="handleaddArchivePost" icon="upload">
					上传</a-button>
				<a-button class="cu-btn-danger" v-if="(tag && tag != 'fdsFile' && !type) || type != 1" icon="delete"
					@click="handleSub($refs.table.selectedRowKeys)">删除</a-button>
			</template>
		</tool-bar>
		<DataGrid ref="table" :columns="columns" :url="url" :queryParam="queryParam" rowKey="uuid"
			...
			<template slot="action" slot-scope="{ text, record }">
              ...
              <a @click="getViewUrlWebPath(record)">在线编辑</a>
						...
						<a-divider type="vertical" />
						<a v-if="tag && tag === 'fdsFile'" @click="handleSub(record, { fileUrl: record.url })">删除</a>
		...
		<!-- 上传文件 -->
		<web-upload ref="createPostModal" :visible="visible" :url="url.webUpload" :loading="confirmLoading" :data="mdl"
			@cancel="
        () => {
          visible = false;
        }
      " @ok="handleOkUserPost" @resTable="resTable" />
	</a-card>
</template>
		...
		methods: {
			...
			handleDownLoad(e) {
				if (this.label === "attachment") {
					...
					downloadMinio('vue-accessory', e.fileName);
				}
			},
