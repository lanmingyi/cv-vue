...
export default {
  name: "Table",
  components: { DataGrid, webUpload, previewModal },
  ...
  methods: {
    ...
    handlePreview(e) {
      ...
      this.$nextTick(() => {
        ...
        const flag = getTypeByList(medieaType, e.filePath);
        const video = getTypeByList(videoType, e.filePath);
        if (flag) {
          ...
          pdfStreamHandeler("/document/minio/pdfStreamVideo", {
            fileuUrl: this.queryParamDicItem.bucket + "/" + e.filePath,
          }).then((res) => {
          ...
          this.$refs.previewModal.fileUrl =
            this.kkFileView +
            "?url=" +
            encodeURIComponent(Base64.encode(baseUrl + e.filePath)) +
            "&watermarkTxt=" +
